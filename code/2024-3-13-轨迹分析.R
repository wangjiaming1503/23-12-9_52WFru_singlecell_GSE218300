...
save(sce,
    file = paste0("sceobject/", get_time(), "-SCE-combined_seurat-SCVI-intergraed-umap-add-learn-N4-slm-louvain-muti-leiden_all_joined-tsne-anotated-alltsne.Rdata")
)
...
milo_obj <- countCells(milo_obj, samples="MouseRNA_SingleR.labels", meta.data=colData(sce))
milo.obj  <- milo_obj
save(milo_obj,
file = paste0("./sceobject/",get_time(),"miloobj-all_joined-tsne-anotated-alltsne.Rdata"))
#需要把分组插入标注中
milo.design <- as.data.frame(xtabs(~ MouseRNA_SingleR.labels + group, data=milo.meta))
milo.design <- milo.design[milo.design$Freq > 0, ]
rownames(milo.design) <- milo.design$MouseRNA_SingleR.labels
milo.design <- milo.design[colnames(nhoodCounts(milo.obj)),]

milo.res <- testNhoods(milo.obj, design=~MouseRNA_SingleR.labels + group, design.df=milo.design)
head(milo.res)
...
milo_obj <- countCells(milo_obj, samples = "MouseRNA_SingleR.labels", meta.data = colData(sce))
da_results <- testNhoods(milo_obj, design = ~MouseRNA_SingleR.labels + group )
#milo_obj <- calcNhoodDistance(milo_obj, d=30)



milo_obj <- countCells(milo_obj, samples = "orig.ident", meta.data = colData(sce))


da_results <- testNhoods(milo_obj, design = ~ orig.ident + group ,data = colData(sce))
...
genesKept <- geneFiltering(as.matrix(sc_obj@assays$RNA@counts), scenicOptions,
...
cell.dist <- as.dist(1 - armaCor(t(ldata)))
show.velocity.on.embedding.cor(scv,
    n = 200, scale = "sqrt", cell.colors = ac(traj.col, alpha = 0.5),
...
sds <- getLineages(sc_obj, reducedDim = "dm", clusterLabels = clusters, start.clus = "MP")
