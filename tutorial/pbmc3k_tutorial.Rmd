...
load("~/work/2023-12-9_52WFru_singlecell_GSE218300/SeuratObjects/Seurat_GSM6738781_WD_4538.RData")
pbmc <- GSM6738781_WD_4538
pbmc.data <- GetAssayData(pbmc, assay = "RNA", slot = "counts")
```
...
pbmc.data[c("Xkr4","Gm1992","Rp1"), 1:10]
```
...
# saveRDS(pbmc, file = "GSM6738781_WD_4538_analyzed.rds")
```
...
VlnPlot(pbmc, features = c("Csf1r", "Cd68"))
# you can plot raw counts as well
VlnPlot(pbmc, features = c("Csf1r", "Cd68"), slot = 'counts', log = TRUE)
```

```{r, fig.height=8, fig.width=10}
FeaturePlot(pbmc, features = c("Csf1r", "Cd68"))
```
...
`{# {r labelplot, fig.height=5, fig.width=9} # new.cluster.ids <- c("Naive CD4 T", "CD14+ Mono", "Memory CD4 T", "B", "CD8 T", "FCGR3A+ Mono", "NK", "DC", "Platelet") # names(new.cluster.ids) <- levels(pbmc) # pbmc <- RenameIdents(pbmc, new.cluster.ids) # DimPlot(pbmc, reduction = 'umap', label = TRUE, pt.size = 0.5) + NoLegend()`
...
`{# {r save.rds, eval=FALSE} # saveRDS(pbmc, file = "../output/pbmc3k_final.rds")`

`{# {r save.times, include = FALSE} # write.csv(x = t(as.data.frame(all_times)), file = "../output/timings/pbmc3k_tutorial_times.csv")`
